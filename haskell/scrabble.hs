...
-------------------------------------------------------------------------
minNothingDistance :: Board -> Int -> Int
minNothingDistance [] initialMin = initialMin
minNothingDistance (c:b) initialMin | min < initialMin = (minNothingDistance b min)
                                    | otherwise = (minNothingDistance b initialMin)
                                    where min = length (takeWhile (\arg -> arg == Nothing) c)

smoothBoard :: Board -> Int -> Board
smoothBoard [] min = []
smoothBoard (c:b) min = (nothingsList ++ newColumn):(smoothBoard b min)
               where nothingsList = (take 7 (repeat Nothing))
                     newColumn = drop min c

columnHeads :: Board -> [Maybe Char]
columnHeads [] = []
columnHeads b = [head c | c <- b]

columnTails :: Board -> Board
columnTails [] = []
columnTails b = [tail c | c <- b]

emptyBoard :: Board -> Bool
emptyBoard [] = True
emptyBoard (c:b) | c == [] = emptyBoard b
                 | otherwise = False

turnClockwise :: Board -> Board
turnClockwise b | emptyBoard b = []
                | otherwise = (columnHeads b):(turnClockwise (columnTails b))

addUp :: Board -> Board
addUp b = smoothBoard b min
          where min = minNothingDistance b maxBound

addDown :: Board -> Board
addDown b = map reverse auxBoard
            where auxBoard = addUp (map reverse b)
     

addLeft :: Board -> Board
addLeft b = turnClockwise (turnClockwise (turnClockwise auxBoard))
            where auxBoard = addUp (turnClockwise b)

addRight :: Board -> Board
addRight b = turnClockwise (turnClockwise (turnClockwise auxBoard))
             where auxBoard = addDown (turnClockwise b)
------------------------------------------------------------------------------------------
autoResize :: Board -> Board
autoResize = addRight.addLeft.addDown.addUp
----------------------------------------------------------------------------------------
